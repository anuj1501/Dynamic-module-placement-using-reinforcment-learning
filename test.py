...
def create_application(name):
    ...
    m_egg = Message("M.Action", "Generator", "Actuator",
...
def main(simulated_time):
    ...
    sorted_clustMeasure = sorted(
        centrality.items(), key=operator.itemgetter(1), reverse=True)
    ...
    dDistribution = deterministicDistributionStartPoint(
        3000, 300, name="Deterministic")
    dDistributionSrc = deterministicDistribution(name="Deterministic", time=10)
    pop1 = Evolutive(top20_devices, number_generators,
                     name="top", activation_dist=dDistribution)
    pop1.set_sink_control({"app": app1.name, "number": 1,
    ...
    pop2.set_sink_control(
        {"id": main_fog_device, "number": number_generators, "module": app2.get_sink_modules()})
