...
class Evolutive(Population):
            ...
            for item in range(self.number_generators):
                ...
                for number in range(ctrl["number"]):
                    idsrc = sim.deploy_source(
                        app_name, id_node=id, msg=msg, distribution=dst)
    ...
    def run(self, sim):
        if len(self.fog_devices) > 0:
            ...
            del self.fog_devices[0]
            self.logger.debug(
                "Activiting - RUN - Evolutive - Deploying a new actuator at position: %i" % fog_device)
            for ctrl in self.sink_control:
                module = ctrl["module"]
                app_name = ctrl["app"]
                for number in range(ctrl["number"]):
    ...
    def initial_allocation(self, sim, app_name):
        ...
        for ctrl in self.src_control:
            ...
            for item in range(self.number_generators):
                ...
                for number in range(ctrl["number"]):
                    idsrc = sim.deploy_source(
                        app_name, id_node=id, msg=msg, distribution=dst, param=param)
