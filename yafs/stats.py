...
class Stats:
    ...
    def compute_times_df(self):
        ...
        self.df["time_latency"] = self.df["time_reception"] - \
            self.df["time_emit"]
        self.df["time_wait"] = self.df["time_in"] - \
            self.df["time_reception"]  #
        self.df["time_service"] = self.df["time_out"] - self.df["time_in"]
        self.df["time_response"] = self.df["time_out"] - \
            self.df["time_reception"]
        self.df["time_total_response"] = self.df["time_response"] + \
            self.df["time_latency"]
    ...
    def average_loop_response(self, time_loops):
        ...
        resp_msg.columns = ['_'.join(col).strip()
                            for col in resp_msg.columns.values]
        results = []
        ...
        for loop in time_loops:
            ...
            for msg in loop:
                try:
                    total += resp_msg[resp_msg.index ==
                                      msg].time_total_response_mean[0]
                except IndexError:
                    total += 0
    ...
    def get_watt(self, totaltime, topology, by=Metrics.WATT_SERVICE):
        ...
        if by == Metrics.WATT_SERVICE:
            ...
            for node_key in nodeInfo:
                ...
                uptime = end-start
                results[node_key] = {"model": nodeInfo[node_key]["model"], "type": nodeInfo[node_key]
                                     ["type"], "watt": uptime*nodeInfo[node_key]["WATT"], "uptime": uptime}
    ...
    def showResults(self, total_time, topology, time_loops=None):
        ...
        for node in values:
            print ("\t\t%i - %s :\t %.2f" %
                   (node, values[node]["model"], values[node]["watt"]))
    ...
    def get_df_modules(self):
        g = self.df.groupby(["module", "DES.dst"]).agg(
