...
class Topology:
    ...
    def load(self, data):
        ...
        for edge in data["link"]:
            self.G.add_edge(edge["s"], edge["d"],
                            BW=edge[self.LINK_BW], PR=edge[self.LINK_PR])
    ...
    def load_graphml(self, filename):
        ...
        for k in self.G.nodes():
            # it has "id" att. TODO IMPROVE
            self.nodeAttributes[k] = self.G.node[k]
    ...
    def draw_png(self, path_file):
        ...
        mt.pyplot.close(fig)  # close the figure
